...
describe('TC-AI-003 — a category-scoped question (EXPECTED FAIL)', () => {
  it('extracts COMPLETION_RATE scoped to the SPECIALIST_REFERRAL category', async () => {
    const documentedIntent: AiIntent = { type: 'COMPLETION_RATE', category: 'SPECIALIST_REFERRAL' };
    ...
    const intent = await engine?.extractIntent('How are referrals doing?');

    expect(
      intent,
      'AI-2: a vague category question ("referrals") must resolve to the correct category scope, not a vague metric',
