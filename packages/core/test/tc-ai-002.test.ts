...
describe('TC-AI-002 — a count question (EXPECTED FAIL)', () => {
  it('extracts PATIENTS_NEEDING_ATTENTION with no period, since §13 defines it as a snapshot', async () => {
    const documentedIntent: AiIntent = { type: 'PATIENTS_NEEDING_ATTENTION' };
    ...
    const intent = await engine?.extractIntent('How many patients need attention?');

    expect(
      intent,
      'AI-2: PATIENTS_NEEDING_ATTENTION is a snapshot metric — attaching a period would silently change its meaning',
    ).toEqual(documentedIntent);

    expect(
      intent?.periodDays,
      'a snapshot metric must never carry a periodDays field',
    ).toBeUndefined();
  });
});
